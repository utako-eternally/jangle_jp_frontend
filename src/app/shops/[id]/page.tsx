...
import { Shop, ShopImage, DAY_OF_WEEK_SHORT_LABELS } from "@/types/models";
...
export default function ShopPublicPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // ギャラリー画像取得（有料プランの場合）
        const isPaid = shopResponse.data.activePlan?.plan_type === 'paid' && 
  ...
  // プラン判定（より安全な判定）
  const isPaidPlan = shop?.activePlan?.plan_type === 'paid' && 
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="bg-white rounded-lg shadow-xl p-6 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-4 text-gray-700">
            ...
            {shop.nearest_station && (
              <div className="flex items-center">
                ...
                <span className="font-medium">
                  {shop.nearest_station.name}駅徒歩{shop.nearest_station.walking_minutes}分
                </span>
              </div>
            )}
            ...
            {shop.today_business_hour && (
              <div className="flex items-center gap-2">
                ...
                <span className={`font-medium ${
                  shop.today_business_hour.is_open_now ? 'text-green-600' : 'text-gray-600'
                }`}>
          ...
          {((shop.frees && shop.frees.length > 0) || shop.set) && (
            <div className="flex flex-wrap gap-2 mb-3">
              {shop.frees && shop.frees.map((free) => {
      ...
      {shop.description && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4">店舗について</h2>
                ...
                <thead>
                  <tr className="border-b-2 border-gray-200">
                    {shop.business_hours
                      ...
                      .map((hour) => (
                        <th key={hour.day_of_week} className="px-2 py-2 text-center text-sm font-semibold text-gray-700">
                ...
                <tbody>
                  <tr>
                    {shop.business_hours
                      .filter(hour => hour.day_of_week <= 6)
                      .map((hour) => (
                        <td key={hour.day_of_week} className={`px-2 py-3 text-center text-sm border-r border-gray-100 last:border-0 ${
                          hour.is_closed ? 'text-red-600 font-medium' : 'text-gray-900'
                        }`}>
                          {hour.display_text}
                        </td>
                      ))}
                    {shop.business_hours.find(h => h.day_of_week === 7) && (
                      <td className={`px-2 py-3 text-center text-sm ${
                        shop.business_hours.find(h => h.day_of_week === 7)?.is_closed 
                          ? 'text-red-600 font-medium' 
                          : 'text-gray-900'
                      }`}>
                        {shop.business_hours.find(h => h.day_of_week === 7)?.display_text}
                      </td>
                    )}
                  </tr>
                </tbody>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-6">アクセス・連絡先</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              {/* 最寄り駅詳細 */}
              {(shop.nearest_station || (shop.sub_stations && shop.sub_stations.length > 0)) && (
                <div>
                  <div className="flex items-start mb-2">
                    <Train className="w-5 h-5 mr-2 text-blue-600 mt-0.5 flex-shrink-0" />
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-900 mb-2">最寄り駅</h3>
                      
                      {/* メイン駅 */}
                      {shop.nearest_station && (
                        <div className="mb-2">
                          <p className="text-gray-900 font-medium">
                            {shop.nearest_station.line_name} {shop.nearest_station.name}
                          </p>
                          {shop.nearest_station.walking_minutes && (
                            <p className="text-sm text-gray-600">
                              徒歩 約{shop.nearest_station.walking_minutes}分
                              {shop.nearest_station.distance_km && (
                                <span className="ml-2">({shop.nearest_station.distance_km.toFixed(1)}km)</span>
                              )}
                            </p>
                          )}
                        </div>
                        ...
                        <div className="space-y-1 text-sm">
                          {shop.sub_stations.map((station, index) => (
                            <p key={index} className="text-gray-600">
                              ...
                              {station.walking_minutes && ` 徒歩${station.walking_minutes}分`}
                            </p>
                          ))}
                        </div>
                      )}
              ...
              {shop.website_url && (
                <div>
                  <div className="flex items-start">
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-900 mb-1">ウェブサイト</h3>
                      <a
      ...
      {/* ========================================
          6. LINE連携（有料プランのみ）
      ======================================== */}
      {isPaidPlan && (shop.line_official_id || shop.line_add_url || shop.line_qr_code_url) && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
          <div className="bg-gradient-to-r from-green-50 to-green-100 rounded-lg shadow p-6 border border-green-200">
            <h2 className="text-xl font-bold text-gray-900 mb-4">LINE公式アカウント</h2>
              ...
              <div className="space-y-4">
                {shop.line_official_id && (
                  <div>
                    ...
                    <p className="text-lg font-semibold text-gray-900">{shop.line_official_id}</p>
                  </div>
                )}
      ...
      {shop.services && shop.services.length > 0 && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-6">
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-4">提供サービス</h2>
                    ...
                    <thead>
                      <tr className="border-b-2 border-gray-300">
                        {Object.entries(groupedRules).map(([category]) => (
                          <th key={category} className="px-4 py-3 text-left text-sm font-semibold text-gray-700 bg-gray-50">
                            {category}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      <tr>
                        {Object.entries(groupedRules).map(([category, rules]) => (
                          <td key={category} className="px-4 py-3 align-top border-r border-gray-200 last:border-r-0">
                            <div className="space-y-1">
                              {rules.map((rule) => (
                                <div key={rule.id} className="text-sm text-gray-900">
                    ...
                    <div key={category}>
                      ...
                      <div className="space-y-3">
                        {menus.map((menu) => (
                          ...
                          >
                            <div className="flex-1">
                              <h4 className="font-medium text-gray-900">{menu.item_name}</h4>
                              {menu.description && (
      ...
      {/* ========================================
          10. ブログ記事一覧（未実装予定のため準備のみ）
      ======================================== */}
      {/* TODO: ブログAPIが実装されたら追加 */}
